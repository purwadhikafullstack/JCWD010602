...
const authControllers = {
  authRegistration: async (req, res) => {
    ...
    try {
      ...
      if (checkEmailPhoneNumber) {
        return res
          .status(201)
          .json({ message: "Email atau Nomor Handphone sudah terdaftar" });
      }
  ...
  authRegistrationSocialMedia: async (req, res) => {
    const t = await sequelize.transaction();
    try {
      const {
        fullname,
      ...
      const createUserSocial = await User.create({
        fullname: fullname,
        email: email,
        phoneNumber: phoneNumber,
        isVerified: isVerified,
      ...
      res.status(200).json({
        message: "Anda berhasil registrasi melalui Facebook Account",
      });
    } catch (err) {
      await t.rollback();
      console.log(err);
  ...
  resendVerification: async (req, res) => {
    try {
      ...
      if (
        !userData.User.dataValues.isVerified &&
        !userData.User.dataValues.suspend
      ) {
        if (countAttempts >= 5) {
          await User.update(
            { suspend: _1day },
